...
function fabric_ca() {
  ...
  # PRIV_KEY=$(ls *_sk)
  PRIV_KEY=$(docker run --rm -it -v sshvolume:/root/fabric-artifacts busybox ls /root/fabric-artifacts/crypto-config/peerOrganizations/org1.example.com/ca | grep _sk)
